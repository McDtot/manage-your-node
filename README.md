...
[v0.6.0](https://github.com/McDtot/manage-your-node/releases/tag/v0.6.0) 支持订阅内节点自命名，并精简为真实部署流程：

- 每条订阅可以为普通节点单独设置显示名称，同一节点在不同订阅中可使用不同名称
- 自定义名称同时应用于 Base64 与 Mihomo / Clash 输出，不会修改 3x-ui 客户端或原始分享链接
- 移除 Dry Run，节点部署与代理链下发仅保留 Native SSH 真实执行流程
- 已有数据库会自动迁移显示名称字段，并将旧模拟部署标记为不可用
