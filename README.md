...
[v0.4.0](https://github.com/McDtot/manage-your-node/releases/tag/v0.4.0) 将访问账号统一称为“用户”，并完善一节点多用户管理：

- 用户按节点分组展示，节点卡片可直接添加用户并显示用户数量
- 同一节点可创建多名用户，每名用户拥有独立 UUID、流量额度和到期时间
- 创建用户后保留当前节点选择，便于连续添加；同一节点禁止重名用户
- 后端继续兼容原有 `clients` 数据表和 API，升级不需要迁移已有数据
...
| 用户 | 在同一节点创建多个独立用户，支持启停、重置流量、修改额度与到期时间 |
| 订阅 | 默认订阅与自定义订阅链接分发 |
...
7. 可选创建首个用户并拉取真实分享链接；之后可继续在同一节点添加其他用户
